...
const ProjectDetailsViewerPage: React.FC = () => {
  ...
  const { 
    ...
    if (!project || !user) {
      return {
        isOwner: false,
        isAcceptedContributor: false,
        hasApplied: false,
        canApply: false,
        canChat: false,
        canManage: false,
        viewerApplicationStatus: undefined
      };
    }
  ...
  // Fetch messages only when permissions are confirmed
  useEffect(() => {
  ...
  // Optimized realtime messages subscription
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .subscribe((status, err) => {
        if (status === 'SUBSCRIBED') {
          ...
          addNotification(
            'Realtime chat connection failed. Messages may not update automatically.',
  ...
  const handleMarkMilestoneComplete = async (milestoneId: string) => {
    ...
    try {
      const { error } = await updateMilestoneStatus(
        milestoneId, 
        projectId, 
        MilestoneStatus.COMPLETED,
        project.title,
        project.posted_by_user_id,
  ...
  const displayUserName = project.posted_by_user?.is_anonymous && 
    !isOwner && 
    !isAcceptedContributor 
      ? 'Anonymous Researcher' 
      : project.posted_by_user?.name;

  const displayUserInstitution = project.posted_by_user?.is_anonymous && 
    !isOwner && 
    !isAcceptedContributor 
      ? '' 
      : `(${project.posted_by_user?.institution || 'Unknown'})`;
  ...
  return (
    <div className="container mx-auto p-4 md:p-8">
      <div className="bg-white shadow-xl rounded-lg overflow-hidden">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <p className="text-gray-600 flex items-center">
                  {project.compensation_model === CompensationModel.STIPEND ? 
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {project.required_skills.map(skill => (
                    <span key={skill} className="bg-primary-light text-primary-dark px-3 py-1 rounded-full text-sm">
                      {skill}
                    </span>
                  ))}
                </div>
                ...
                <div className="space-y-3">
                  {project.milestones.map(milestone => (
                    <MilestoneItem 
                      ...
                      milestone={milestone} 
                      isEditable={isOwner}
                      onFund={isOwner ? () => handleFundMilestone(milestone) : undefined}
                      onMarkComplete={isOwner ? () => handleMarkMilestoneComplete(milestone.id) : undefined}
                    />
                  ))}
                </div>
              </div>
          ...
          <aside className="lg:col-span-1 space-y-6">
            ...
            {hasApplied && !isOwner && (
              <div className="p-4 bg-green-50 border border-green-200 rounded-md text-green-700 text-center">
                You have applied to this project. Status: <strong>{viewerApplicationStatus}</strong>
              </div>
            )}
            ...
            <div className="p-4 bg-gray-50 rounded-lg border">
              ...
              <div className="flex items-center space-x-3">
                <img 
                  src={project.posted_by_user?.profile_photo_url || 
                    `https://ui-avatars.com/api/?name=${displayUserName?.replace(' ','+') || 'R L'}&background=0D8ABC&color=fff`} 
                  alt={displayUserName || 'Research Lead'} 
              ...
              {messages.map(msg => (
                <div key={msg.id} className={`flex ${msg.sender_user_id === user?.id ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-xs lg:max-w-md p-3 rounded-lg shadow ${
                    msg.sender_user_id === user?.id ? 'bg-primary text-white' : 'bg-white text-gray-800 border'
                  }`}>
                    <p className="text-xs font-semibold mb-0.5 opacity-80">
                      {msg.sender_user?.name || 'User'} {msg.sender_user_id === user?.id ? '(You)' : ''}
                    </p>
                    ...
                    {msg.attachment_url && (
                      <a 
                        ...
                        className={`mt-1 text-xs flex items-center ${
                          msg.sender_user_id === user?.id ? 'text-blue-200 hover:text-blue-100' : 'text-primary hover:text-primary-dark'
                        }`}
                      >
        ...
        {/* Applications Management (for owner) */}
        {isOwner && project.id && (
          <div className="p-6 md:p-8 border-t">
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        <h4 className="font-semibold text-primary">
                          {app.contributor_user?.name || 'Applicant'}
                        </h4>
                        <p className="text-sm text-gray-500">
                          {app.contributor_user?.email || 'No email'}
                        </p>
                        <p className="text-xs text-gray-400 mt-0.5">
                          Applied: {new Date(app.created_at || Date.now()).toLocaleDateString()}
                        </p>
                      </div>
                    ...
                    {(app.status === ApplicationStatus.PENDING || app.status === ApplicationStatus.SHORTLISTED) && (
                      <div className="mt-3 flex space-x-2 flex-wrap gap-y-2">
                        <Button 
                          size="sm" 
                          variant="primary" 
                          onClick={() => handleApplicationStatusUpdate(app, ApplicationStatus.ACCEPTED)} 
                        ...
                        {app.status === ApplicationStatus.PENDING && (
                          <Button 
                            size="sm" 
                            variant="outline" 
                            onClick={() => handleApplicationStatusUpdate(app, ApplicationStatus.SHORTLISTED)}
                          >
                            Shortlist
                          </Button>
                        )}
                        <Button 
                          size="sm" 
                          variant="danger" 
                          onClick={() => handleApplicationStatusUpdate(app, ApplicationStatus.REJECTED)} 
