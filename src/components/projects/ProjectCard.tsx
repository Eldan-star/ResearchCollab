...
const ProjectCard: React.FC<ProjectCardProps> = ({ project, showApplyButton = true }) => {
  ...
  // Compensation icon with warning for unknown models
  const getCompensationIcon = (model?: CompensationModel) => {
    switch (model) {
      case CompensationModel.STIPEND: 
        return <DollarSign size={16} className="mr-1 text-green-600" />;
      case CompensationModel.CO_AUTHORSHIP: 
        return <Users size={16} className="mr-1 text-blue-600" />;
      case CompensationModel.LETTER_OF_RECOMMENDATION: 
        return <CheckSquare size={16} className="mr-1 text-purple-600" />;
      default:
        if (model) console.warn('Unknown compensation model:', model);
        return <Briefcase size={16} className="mr-1 text-gray-600" />;
    }
  };
      ...
      <div className="p-6 flex-grow">
        ...
        <div className="text-sm text-gray-500 mb-3">
          ...
          {project.application_deadline && (
            <p className="flex items-center">
              ...
              Apply by: {formatDeadline(project.application_deadline)}
            </p>
          )}
        </div>
        ...
        {project.posted_by_user && (
          <div className="text-xs text-gray-500 mt-auto pt-3 border-t border-gray-200">
            ...
            {project.posted_by_user.is_anonymous && !isOwner ? '' : ` (${project.posted_by_user.institution || 'Unknown'})`}
          </div>
        )}
      </div>
